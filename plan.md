...
| **Primary interaction** | Hybrid: **voice for mesh edits**, **hand gestures for camera nav** (orbit/zoom/pan) | Voice-point is reliable for edits; gestures handle continuous 3D navigation. |
...
#### [~] Step 3 — Hand-as-cursor calibration → region picking (Spike S3)  *(code done + math verified; ≥90% gate needs your webcam)*
**Goal:** point at the Blender viewport and select the object under your finger.
**Files:** `client/cursor/{types,displays,webcam_tracker,mapper,provider,ui_overlay}.py` (lifted ~verbatim from Wand), `client/cursor/viewport.py` (NEW — screen→region conversion), `addon/forge_addon.py` (`get_window_geometry`), `scripts/spike_pick.py`, `tests/test_viewport.py`.
**Design note — pivot:** the first cut mapped fingertip→region directly (stability-dwell, no overlay) to avoid needing the window position. In practice that was hard to calibrate (no visual feedback). Switched to **Wand's screen calibration**: the hand drives a visible dot, the user moves it into 4 corner rings (easy, proven). The hand→**screen** homography is then converted to region pixels via `ViewportScreenRect`, which folds in HiDPI `pixel_size` (2.0 on Retina) using `get_window_geometry` (window pts + region px). This *does* use the window position, but it's read live from the addon — fine.
**Auto-verified (9 tests):** screen→region conversion reproduces region corners/center exactly from the real measured geometry (window 1512×838 pt, region 2477×1458 px @ ps 2.0); all cursor modules import incl. the AppKit overlay; addon registers with `get_window_geometry`.
**Pending (needs you):** **reload the addon** (it gained a command — quit & reopen Blender, enable Forge MCP, click Connect), then `python scripts/spike_pick.py --target Cube`. Move the yellow dot into the 4 rings to calibrate, then point at the cube → confirm **≥90%**.
